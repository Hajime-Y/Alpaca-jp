...
def encode_prompt(prompt_instructions):
    ...
    prompt = open("./prompt_jp.txt").read() + "\n"
...
def post_process_ms_response(num_prompt_instructions, response):
    if response is None:
        return []
    ...
    for idx, inst in enumerate(raw_instructions):
        ...
        if len(inst) <= 6 or len(inst) > 300:
            print("filter out too short or too long instructions. instruction lengths: ", len(inst.split()))
            continue
        ...
        #     print("filter those starting with non-english character")
        #     continue
...
def generate_instruction_following_data(
    ...
    model_name="open-mixtral-8x7b",  # open-mistral-7b, open-mixtral-8x7b, open-mixtral-8x22b
    ...
    while len(machine_instruction_data) < num_instructions_to_generate:
        ...
        decoding_args = utils.MistralDecodingArguments(
        ...
        results = utils.mistralai_completion(
        ...
        for result in results:
            new_instructions = post_process_ms_response(num_prompt_instructions, result)
            instruction_data += new_instructions

        print("ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー")
